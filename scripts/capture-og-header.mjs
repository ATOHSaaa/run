 ...
 * 記事 OGP の青帯（ブログ名）に使う PNG をコミット用パスへ書き出す。
 *
 * 実行: npm run capture:og-header（Puppeteer は devDependencies のみ）
 */
...
const PORT = 4323;
const OUT = path.join(ROOT, 'src/assets/og/site-header-strip.png');
...
async function main() {
  ...
  try {
    ...
    const rawPng = await target.screenshot({ type: 'png' });
    await browser.close();
